...
def main(argv=None):
    ...
    if FLAGS.mode == 'train':
        for itr in xrange(MAX_ITERATION):
            l_image, color_images, heatmaps = batch_reader.next_batch(FLAGS.batch_size)
            feed_dict = {images: l_image, lab_images: color_images, heat_maps: heatmaps, train_phase: True}
        ...
        count = 10
        l_image, color_images, heatmaps = batch_reader.get_random_batch(count)
        feed_dict = {images: l_image, lab_images: color_images, heat_maps: heatmaps, train_phase: False}
