...
class Net(object):
  ...
  def loss_heatmap(self, scope, conv8_313, prior_boost_nongray, gt_ab_313, hm_ab_313):
    
    flat_conv8_313 = tf.reshape(conv8_313, [-1, 313])
    flat_hm_ab_313 = tf.reshape(hm_ab_313, [-1, 313])
    flat_conv8_313 = tf.multiply(flat_conv8_313, flat_hm_ab_313)
    flat_gt_ab_313 = tf.reshape(gt_ab_313, [-1,313])
    g_loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=flat_conv8_313, labels=flat_gt_ab_313)) / (self.batch_size)
    ...
    dl2c = tf.stop_gradient(dl2c)
    #
    #decode_lab = decode(conv8_313, gt_ab_313, 2.63)
    #heatmap_loss = 
