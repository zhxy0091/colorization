...
def evaluation(sess):
    ...
    train_images, testing_images, validation_images = flowers.read_dataset(FLAGS.data_dir, 0.0, 0.2)
    ...
    loss = 0.0
    while (batch_reader.get_batch_offset()+FLAGS.batch_size) <= len(validation_images):
        startIdx = batch_reader.get_batch_offset()
